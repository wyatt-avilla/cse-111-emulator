...
Controller::Controller(Console* console)
    : console(console), controller_state(0) {}
...
void Controller::updateController() {
    ...
    if (keystate[SDL_SCANCODE_Z])
        new_state |= CONTROLLER_A_MASK;
    if (keystate[SDL_SCANCODE_X])
        new_state |= CONTROLLER_B_MASK;
    if (keystate[SDL_SCANCODE_LSHIFT] || keystate[SDL_SCANCODE_RSHIFT])
        new_state |= CONTROLLER_SELECT_MASK;
    if (keystate[SDL_SCANCODE_RETURN])
        new_state |= CONTROLLER_START_MASK;
    if (keystate[SDL_SCANCODE_UP])
        new_state |= CONTROLLER_UP_MASK;
    if (keystate[SDL_SCANCODE_DOWN])
        new_state |= CONTROLLER_DOWN_MASK;
    if (keystate[SDL_SCANCODE_LEFT])
        new_state |= CONTROLLER_LEFT_MASK;
    if (keystate[SDL_SCANCODE_RIGHT])
        new_state |= CONTROLLER_RIGHT_MASK;
