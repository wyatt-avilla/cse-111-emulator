...
Disassembler::Disassembler(const std::string& filename) : filename(filename) {}
...
std::string Disassembler::getNewFilename() {
    ...
    disassembled_filename.erase(disassembled_filename.length() - SLUG_EXT_LENGTH);
    disassembled_filename += ".S";
    return disassembled_filename;
}

void Disassembler::parseInstructions(std::ofstream& disassembled, const std::array<uint8_t, SLUG_FILE_SIZE>& slug_file) {
    const uint16_t setup_address = ((uint16_t) slug_file[SETUP_ADR_LOCATION] << 8 |
                              slug_file[SETUP_ADR_LOCATION + 1]) -
                             SLUG_FILE_SIZE;
    const uint16_t loop_address = ((uint16_t) slug_file[LOOP_ADR_LOCATION] << 8 |
                             slug_file[LOOP_ADR_LOCATION + 1]) -
                            SLUG_FILE_SIZE;
    const uint16_t rom_address = ((uint16_t) slug_file[ROM_ADR_LOCATION] << 8 |
    ...
    for (uint16_t i = MISC_FUNCTIONS_BEGIN; i < rom_address; i += 4) {
        ...
        const auto opcode = (uint8_t) ((instruction & OPCODE_MASK) >> 26);
        const auto reg_a = (uint8_t) ((instruction & REG_A_MASK) >> 21);
        const auto reg_b = (uint8_t) ((instruction & REG_B_MASK) >> 16);
        if (opcode == 62) {
            const auto reg_c = (uint8_t) ((instruction & REG_C_MASK) >> 11);
            const auto shift_value =
                (uint8_t) ((instruction & SHIFT_VALUE_MASK) >> 6);
...
void Disassembler::parseData(std::ofstream& disassembled, const std::array<uint8_t, SLUG_FILE_SIZE>& slug_file) {
    const uint16_t rom_address = ((uint16_t) slug_file[ROM_ADR_LOCATION] << 8 |
                            slug_file[ROM_ADR_LOCATION + 1]) -
                           SLUG_FILE_SIZE;
    uint32_t data_size =
        ((uint32_t) slug_file[DATA_SIZE_LOCATION] << 24 |
         slug_file[DATA_SIZE_LOCATION + 1] << 16);
    data_size |=
        ((uint16_t) slug_file[DATA_SIZE_LOCATION + 2] << 8 |
