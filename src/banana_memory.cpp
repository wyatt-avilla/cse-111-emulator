...
uint16_t BananaMemory::l16u(uint16_t load_address) const {
    ...
    if (load_address & 1) {
        ...
        std::cerr << "warning trying to read word on a false word adress" << std::endl;
...
uint32_t BananaMemory::loadInstruction(uint16_t load_address) const {
    ...
    out = (l8u(load_address)<<24) | (l8u(load_address+1) << 16) | (l8u(load_address+2) << 8) | (l8u(load_address+3));
    return out;
}
...
void BananaMemory::w16u(uint16_t address, uint16_t value) {
    if (address & 1) {
        std::cerr << "warning: trying to write word on an unaligned address" << std::endl;
    }
